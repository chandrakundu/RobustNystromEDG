function [Xk, Pk, timer] = SREDGAAP_EXP2(data, params)
    ...
    % para.tol = get_field(params, 'rpca_tol', 1e-4); % tolerance for RPCA
    % [Fk, S0] = AccAltProj(F_obs, r, para );

    % Fk = F_star + randn(size(F_star)) * 10; % initial guess for Fk

    % B_star = operatorB(F_star, E); % true B matrix
    % sv = svds(B_star, d+1); 
    % sv_gap = sv(d) - sv(d+1); % gap between d-th and (d+1)-th singular values
    ...
    for k=1:max_iter
        ...
        % Zk = F_star - ABk; % residual matrix
        % zeta_k = 0.2*max(abs(Zk(:))); % update threshold based on residual
        % Sk_new = T_hardThreshold(Zk, zeta_k); % hard thresholding
        ...
        % rowNorm = sqrt(sum(Rk.^2,2));
        % ak = alpha*m; 
        % [~,idx] = maxk(rowNorm, ak); % support rows
        % Sk_new = zeros(size(Rk)); % initialize Sk_new
        % Sk_new(idx,:) = Rk(idx,:); % set the support rows in Sk_new
